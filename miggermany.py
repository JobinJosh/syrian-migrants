...
class Person:
    ...
    def choose_accommodation(self):
        ...
        elif 2054 <= self.income <= 3666:
            self.accommodation = 'Standard Apartment'
        elif self.income < 2054 and self.social_status == 'Single':
            self.accommodation = random.choice(['Shared Housing', 'Public Housing'])
        elif self.social_status == 'Family' and self.income >= 2054:
            self.accommodation = 'House'
        elif self.income < 2054 and self.social_status == 'Family':
...
def create_persons_and_plot(num_persons, education_probs, employment_probs, income_min, income_max, social_status_probs, relatives_abroad_prob):
    ...
    for _ in range(num_persons):
        age = random.choices(['0-24', '20-45', '45-65',  '65+'], [0.389, 0.482, 0.112, 0.017])[0]
        gender = random.choices(['Female', 'Male'], [0.41, 0.59])[0]
...
def calculate_resource_usage(persons):
    accommodation_needs = {
        'Luxury Apartment': {'water': 125, 'electricity': 1.943, 'land': 65},
        'House': {'water': 125, 'electricity': 1.943, 'land': 55},
        'Standard Apartment': {'water': 125, 'electricity': 1.943, 'land': 46},
        'Shared Housing': {'water': 125, 'electricity': 1.943, 'land': 37},
        'Public Housing': {'water': 125, 'electricity': 1.943, 'land': 28},
...
# Streamlit UI
st.title('War displaced Migrants')
st.subheader('Syrian Migrants')

num_persons = st.number_input('Number of Persons', min_value=1, step=1, value=10000)
income_min = st.number_input('Minimum Salary (Euro)', min_value=0, value=300)
income_max = st.number_input('Maximum Salary (Euro)', min_value=0, value=650)

education_probs = [
    st.slider('No Education Probability', 0.0, 1.0, 0.03),
    st.slider('Primary Education Probability', 0.0, 1.0, 0.36),
    st.slider('Secondary Education Probability', 0.0, 1.0, 0.39),
    st.slider('Higher Education Probability', 0.0, 1.0, 0.25)
]

employment_probs = [
    st.slider('Employed Probability', 0.0, 1.0, 0.64),
    st.slider('Unemployed Probability', 0.0, 1.0, 0.36)
]

social_status_probs = [
    st.slider('Single Probability', 0.0, 1.0, 0.309),
    st.slider('Family Probability', 0.0, 1.0, 0.6)
]
